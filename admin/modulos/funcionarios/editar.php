<?php 
	...
	if(!empty($_POST)){
		...
		if($valida==true){
			...
			if($res==1){
				...
				echo "<h1>Erro ao atualizar funcionário</h1>";
			}
	...
	$sqlCliente = "SELECT 
						id, nome, data_admissao, telefone, email, cidade_id 
					FROM 
						funcionarios
					WHERE 
						id=$id";
...
<form action="" method="post"  class="panel panel-warning">
	...
	<div class="panel-body">
		...
		<div class="form-group">
			...
			<input type="text"  value="<?php echo Conversor::dataBancoParaUsuario($funcionario['data_admissao']); ?>" class="form-control mascaraData" name="fdata_admissao" id="data_nascimento" placeholder="Digite a data de nascimento"  >
		</div>
